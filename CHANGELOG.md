...
### Changed
* fix: the driver no longer sizes reads or allocations from a backend-supplied length without checking it. Every message length is validated against the layout of the message and a cap for its type, the lengths nested inside `DataRow` and `FunctionCallResponse` are checked against the message envelope, and the `NegotiateProtocolVersion` option count is bounded. A hostile or desynced backend could previously drive an allocation of up to two gigabytes from a five byte header [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: the shared read buffer now has a maximum of 32 MiB and grows once per request rather than once per socket read. It previously doubled on every read while a large request was outstanding, so a peer that declared a large length and then sent the bytes slowly grew it step by step until the doubling past 1 GiB overflowed to a negative size [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: a zero-length `CopyData` body is accepted. It is legal on the wire and was previously rejected by an assertion, so it failed only on JVMs started with `-ea` [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
...
* fix: a connection whose stream has been refused is no longer reusable. Any refused length or count marks the stream broken, so `Connection.isClosed()` reports it closed and a pool that tests on borrow discards it instead of handing the next caller a stream that is out of step with the protocol. The socket is dropped with `SO_LINGER 0` [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
...
* fix: the authentication loop, the GSS encryption handshake and the GSS authentication handshake each stop after 64 round trips. A zero-length GSS token is a legal continuation, so a server could previously keep a client exchanging tokens indefinitely before any credentials were exchanged [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
* fix: `ErrorResponse` before authentication is capped at 30000 bytes rather than 32 MiB, matching libpq's limit for messages outside `VALID_LONG_MESSAGE_TYPE`, and a C string inside a declared message is no longer scanned past the end of that message. Both were allocations an unauthenticated peer could force [Issue #4015](https://github.com/pgjdbc/pgjdbc/issues/4015)
